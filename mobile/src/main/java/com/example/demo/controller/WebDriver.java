...
//	public static String WEB_DRIVER_PATH = "/Users/강윤식/Desktop/geckodriver.exe";
	...
	public WebDriver() { 
		//chrome();
		base_url = "https://m.map.naver.com/directions/#/publicTransit/list/,"+start_y+","+start_x+","+start_y+","+start_x+",false,/" +
                ","+end_y+","+end_x+","+end_y+","+end_x+",false,/2";
	           ...
	           Elements[] temp = new Elements[5];
	            ...
	            for(int num=0; num<5; num++) {
	            	...
	            	for(int i=0;i<p.size();i++) {
	            		if(p.get(i).hasAttr("data-route_id")&&p.get(i).attr("data-route_id").equals(Integer.toString(num))) {
	            ...
	            for(int i=0;i<10;i++) {
	            	...
	            	for(int j=0;j<info[i].move.size();j++) {
	            		System.out.println(info[i].move.get(j).time+"?? : "+info[i].move.get(j).type);	
	            	}
	            ...
	            for(int i=0;i<10;i++) {
	            	...
	            	for(int j=0;j<info[i].move.size();j++) {
	            		if(info[i].move.get(j).type=='w') {
	            			...
	            			all[i]=all[i]+info[i].move.get(j).time+"<time> "+info[i].busnum.get(check)+"<busnum> "+info[i].sname.get(check)+"<station> ";
	            			++check;
	            		}
